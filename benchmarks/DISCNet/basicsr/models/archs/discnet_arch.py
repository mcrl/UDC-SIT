...
class DISCNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, ns, kpn_sz=5, norm_type=None,
        ...
        #############################
        # Encoder
        self.conv_11 = conv_block(
            in_nc, nf, kernel_size=kernel_size, act_type=act_type)
        ...
        self.upconv_22 = ResBlock(
            2 * nf, res_scale=res_scale, act_type=act_type)
        self.upconv_23 = ResBlock(
            2 * nf, res_scale=res_scale, act_type=act_type)
        ...
        if self.kernel_cond:
            ...
            # Here: where cond_nc is in_nc (which is 3 in DISCNet) + psf_nc (PCA dimension of PSF)
            self.kconv_11 = conv_block(
                cond_nc, nf, kernel_size=kernel_size, act_type=act_type)
            self.kconv_12 = ResBlock(
                nf, res_scale=res_scale, act_type=act_type)
            self.kconv_13 = ResBlock(
                nf, res_scale=res_scale, act_type=act_type)
            ...
            self.kconv_21 = conv_block(
                nf, 2 * nf, stride=2, kernel_size=kernel_size, act_type=act_type)
            self.kconv_22 = ResBlock(
                2 * nf, res_scale=res_scale, act_type=act_type)
            self.kconv_23 = ResBlock(
                2 * nf, res_scale=res_scale, act_type=act_type)
            ...
            self.kconv_31 = conv_block(
                2 * nf, 4 * nf, stride=2, kernel_size=kernel_size, act_type=act_type)
            self.kconv_32 = ResBlock(
                4 * nf, res_scale=res_scale, act_type=act_type)
            self.kconv_33 = ResBlock(
                4 * nf, res_scale=res_scale, act_type=act_type)
    ...
    def forward(self, x, psf=None):
        ...
        # here: where we concat PSF kernel code
        if self.kernel_cond:
            if self.kernel_cond == 'img':
                cond_x = x
            elif self.kernel_cond == 'psf':
        ...
        if self.multi_scale:
            ...
            upfea2 = self.upconv_23(
                self.upconv_22(self.upconv_21(fea3) + fea2))
        else:
            upfea2 = self.upconv_23(
                self.upconv_22(self.upconv_21(fea3) + fea2))
        ...
        if self.multi_scale:
            ...
            upfea1 = self.upconv_13(self.upconv_12(
                self.upconv_11(upfea2) + fea1))
        else:
            upfea1 = self.upconv_13(self.upconv_12(
                self.upconv_11(upfea2) + fea1))
...
def kernel2d_conv(feat_in, kernel, ksize):
    ...
    feat_in = F.pad(feat_in, (pad_sz, pad_sz, pad_sz, pad_sz),
                    mode="replicate")
