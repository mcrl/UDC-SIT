...
class DataLoaderTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        clean = torch.from_numpy(load_img(self.clean_filenames[tar_index],'UDC_SIT_RAW'))
        noisy = torch.from_numpy((load_img(self.noisy_filenames[tar_index],'UDC_SIT_RAW')))
        
        clean = clean.permute(2,0,1) #feng
        noisy = noisy.permute(2,0,1) #feng
...
class DataLoaderVal(Dataset):
    ...
    def __getitem__(self, index):
      ...
      #  clean = clean.permute(1,2,0 psnr_train_rgb )
       # noisy = noisy.permute(1,2,0)
        clean = clean.permute(2,0,1) #feng
        noisy = noisy.permute(2,0,1) #feng
